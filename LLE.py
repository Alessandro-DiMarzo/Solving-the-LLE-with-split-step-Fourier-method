...
class LLE_sim:
    def __init__(self, eta, detuning, F, T, roundtrips, N):
        self.eta = eta
        self.detuning = detuning
        self.F = F #driving
        self.T = T #fast time window
        self.roundtrips = roundtrips
    
        # Fast time grid 
        self.N = N #fast time grid points
        self.tau = np.linspace(-self.T/2, self.T/2, self.N)
        ...
        self.n = int(self.roundtrips/self.dt)
        self.E_evolution = np.zeros((self.n, self.N), dtype=complex)
        self.t_vals = np.arange(self.n)*self.dt
    def nonlinear_step(self, A):  
        return (- 1 - 1j * self.detuning + 1j  * np.abs(A)**2) * A
    
    def ssfm(self): #solve LLE
        dt = self.dt #slow time step
    ...
    def plot_slow_time_evolution(self):
        ...
        plt.imshow(np.abs(self.E_evolution)**2, aspect='auto',
        extent=[self.tau[0], self.tau[-1], self.t_vals[0], self.t_vals[-1]], cmap='jet' , origin='lower')
    ...
    def animate_slowtime(self):
        ...
        def update(frame):
            axs[0].set_ylim(0, np.max(np.abs(self.E_evolution[frame])**2) + 2)
            line.set_data(fast_time, np.abs(self.E_evolution[frame])**2)
            axs[0].set_title(f'roundtrips: {frame/100}', y = 1.0, pad=-14)
        ...
        frames = range(0,self.n, 21) 
        self.ani = animation.FuncAnimation(fig, update, frames=frames, init_func=init, blit=False, interval=0.001)
        ...
        axs[2].plot(self.t_vals, total_power)
        axs[2].set(xlabel = 'Slow time t (roundtrips)', ylabel = 'Total Power of E' )
        plt.tight_layout()
        
        self.ani.save('slow_time_evolution.gif', writer = 'pillow')
        
    def scan_detuning(self, delta_start, delta_end, delta_steps, x): 
        self.roundtrips = x # change the detuning after x roundtrips
        ...
        def update(frame):
            ...
            axs[1].set_xlim(-2.5, 2.5)
            axs[1].set_ylim(0, np.max(self.get_spectrum(E_profiles[frame])[1]) +2)
        ...
        plt.tight_layout()
        self.ani.save('Multiple-soliton-detuning-scan.gif', writer = 'pillow')
        
        # Fast time intensity profile
        plt.figure()
        plt.title('Fast Time Intensity Profile of |E(t, τ)|² as we Change Detuning')
        plt.imshow(E_profiles, extent=[self.tau[0], self.tau[-1], 
                                delta_values[0], delta_values[-1]], aspect='auto', cmap='hot', origin='lower')
        plt.xlabel('Fast time τ')
...
#%%
sol.animate_slowtime()
#%%
sol.scan_detuning(-2,4,150,5)
